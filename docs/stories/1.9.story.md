...
**Note**: The exact Helmfile values merging syntax depends on Helmfile version. The `needs` field ensures deployment ordering (postgres → api → frontend). Consult `helmfile` docs if the values passthrough syntax above doesn't work — an alternative is to use separate values files per release.
