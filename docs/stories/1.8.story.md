...
- [x] **Task 1: Create Manifest Directory Structure** (AC: 8, 9)
  - [x] Create `infra/k8s/` directory
  - [x] Create `infra/k8s/base/` for all manifest files
  - [x] Create `infra/k8s/base/namespace.yaml` defining `devops-demo` namespace
  - [x] Create `infra/k8s/base/kustomization.yaml` listing all resources

- [x] **Task 2: Create Namespace** (AC: 8)
  - [x] Define `devops-demo` namespace in `namespace.yaml`
  - [x] All subsequent resources must set `namespace: devops-demo`

- [x] **Task 3: Create ConfigMaps** (AC: 4)
  - [x] Create `infra/k8s/base/api-configmap.yaml`
    - `DB_HOST: postgres`
    - `DB_PORT: "5432"`
    - `DB_NAME: devops_demo`
    - `DEBUG: "false"`
  - [x] Create `infra/k8s/base/frontend-configmap.yaml`
    - `NEXT_PUBLIC_API_URL: http://api:8000` (internal cluster URL)

- [x] **Task 4: Create Secrets** (AC: 5)
  - [x] Create `infra/k8s/base/db-secret.yaml`
  ...
  - [x] Add comment reminding that base64 is NOT encryption — this is Phase 1 (K8s Secrets)

- [x] **Task 5: Create PostgreSQL StatefulSet** (AC: 2, 3, 6)
  - [x] Create `infra/k8s/base/postgres-statefulset.yaml`
  ...
  - [x] Create `infra/k8s/base/postgres-service.yaml`
    - Type: ClusterIP
    - Port: 5432 → targetPort 5432
    - Service name: `postgres` (matches DB_HOST in ConfigMap)
  - [x] Create `infra/k8s/base/init-sql-configmap.yaml` from `apps/api/db/init.sql` content
  - [x] Mount init-sql ConfigMap at `/docker-entrypoint-initdb.d/` in the StatefulSet

- [x] **Task 6: Create API Deployment** (AC: 1, 3, 6, 7)
  - [x] Create `infra/k8s/base/api-deployment.yaml`
  ...
  - [x] Create `infra/k8s/base/api-service.yaml`
    - Type: ClusterIP
    - Port: 8000 → targetPort 8000
    - Service name: `api`

- [x] **Task 7: Create Frontend Deployment** (AC: 1, 3, 6)
  - [x] Create `infra/k8s/base/frontend-deployment.yaml`
  ...
  - [x] Create `infra/k8s/base/frontend-service.yaml`
    - Type: ClusterIP
    - Port: 3000 → targetPort 3000
    - Service name: `frontend`

- [x] **Task 8: Create Kustomization File** (AC: 9)
  - [x] Create `infra/k8s/base/kustomization.yaml` listing all resources in order:
    1. namespace.yaml
    2. db-secret.yaml
    3. api-configmap.yaml
    4. frontend-configmap.yaml
    5. init-sql-configmap.yaml
...
- [x] **Task 9: Add Makefile Targets** (AC: 9, 10)
  - [x] Add `k8s-deploy` target: `kubectl apply -k infra/k8s/base/`
  - [x] Add `k8s-delete` target: `kubectl delete -k infra/k8s/base/`
  - [x] Add `k8s-status` target: `kubectl get all -n devops-demo`
  - [x] Add `k8s-load-images` target: load Docker images into Kind
  - [x] Add `k8s-port-forward` target: port-forward API (8000) and Frontend (3000)

- [x] **Task 10: Test Deployment** (AC: 9, 10)
  - [x] Build Docker images (from Story 1.5)
  - [x] Load images into Kind: `kind load docker-image devops-api:0.1.0 devops-frontend:0.1.0 --name devops-demo`
  - [x] Deploy: `kubectl apply -k infra/k8s/base/`
  - [x] Wait for pods to reach Running/Ready
  - [x] Port-forward API: `kubectl port-forward svc/api 8000:8000 -n devops-demo`
  - [x] Port-forward Frontend: `kubectl port-forward svc/frontend 3000:3000 -n devops-demo`
  - [x] Verify `curl http://localhost:8000/health` returns healthy
  - [x] Verify `curl http://localhost:8000/ready` returns 200 (DB connected)
  - [x] Clean up: `kubectl delete -k infra/k8s/base/`
...
- **`NEXT_PUBLIC_API_URL` is a build-time variable** — Next.js bakes `NEXT_PUBLIC_*` env vars at build time (set via Dockerfile ARG in Story 1.5 as `http://localhost:8000`). The ConfigMap value `http://api:8000` will be available to server-side code but won't override the client-side bundle. With port-forward, `http://localhost:8000` still works for browser requests, so this is fine for this story.
- **`readOnlyRootFilesystem` on API** — Python/uvicorn may need `/tmp` for temporary files. If the API pod fails with read-only FS errors, add an `emptyDir` volume mounted at `/tmp`.
...
**Modified:**
- `Makefile` — Added k8s-deploy, k8s-delete, k8s-status, k8s-load-images, k8s-port-forward targets

### Debug Log References

1. **Secret base64 encoding error** — User wrote `base64(app_user)` literally instead of actual base64 encoded values. Fixed by replacing with proper base64 strings.
2. **Frontend CreateContainerConfigError** — `runAsNonRoot: true` failed because image uses non-numeric user `nextjs`. Fixed by adding `runAsUser: 1001` to securityContext.
3. **Port mismatches** — Initial api-deployment.yaml and frontend-deployment.yaml had incorrect ports (8080 instead of 8000, 80 instead of 3000). Corrected during review.

### Completion Notes

- All 12 Kubernetes manifests created in `infra/k8s/base/`
- Kustomize configuration allows deployment with `kubectl apply -k`
- Makefile targets provide convenient commands for deploy/delete/status/port-forward
- PostgreSQL StatefulSet includes PGDATA env var to avoid `lost+found` issues
- All pods successfully deployed and tested with port-forward
- Health checks verified: `/health` and `/ready` endpoints responding correctly

### Change Log

| Date       | Change                                      |
| ---------- | ------------------------------------------- |
| 2026-02-04 | Created all K8s manifests                   |
| 2026-02-04 | Added Makefile targets for K8s operations   |
| 2026-02-04 | Fixed Secret base64 encoding                |
| 2026-02-04 | Fixed Frontend securityContext (runAsUser)  |
| 2026-02-04 | Tested full deployment on Kind cluster      |
...
**Overall: Good** - All 12 Kubernetes manifests are well-structured and follow K8s best practices. The implementation meets all 10 acceptance criteria. Minor naming inconsistencies identified but non-blocking.

### Requirements Traceability

| AC | Requirement | Implementation | Coverage |
|----|-------------|----------------|----------|
| 1 | Deployment manifests for API/Frontend | `api-deployment.yaml`, `frontend-deployment.yaml` | ✅ Complete |
| 2 | StatefulSet for PostgreSQL | `postgres-statefulset.yaml` with volumeClaimTemplate | ✅ Complete |
| 3 | Service manifests (ClusterIP) | `api-service.yaml`, `frontend-service.yaml`, `postgres-service.yaml` | ✅ Complete |
| 4 | ConfigMaps for non-sensitive config | `api-configmap.yaml`, `frontend-configmap.yaml` | ✅ Complete |
| 5 | Secrets for DB credentials (base64) | `db-secret.yaml` with proper encoding | ✅ Complete |
| 6 | Resource requests/limits defined | All workloads have requests/limits | ✅ Complete |
| 7 | Liveness/readiness probes for API | `/health` and `/ready` probes configured | ✅ Complete |
| 8 | All resources namespaced | All set to `devops-demo` | ✅ Complete |
| 9 | kubectl apply -k deploys stack | `kustomization.yaml` + `make k8s-deploy` | ✅ Complete |
| 10 | Accessible via port-forward | `make k8s-port-forward` target | ✅ Complete |
...
- Coding Standards: ✅ YAML well-formatted, consistent indentation
- Project Structure: ✅ `infra/k8s/base/` follows expected pattern
- Testing Strategy: ✅ E2E tests via kubectl + curl as specified
- All ACs Met: ✅ 10/10 acceptance criteria satisfied

### Improvements Checklist

- [x] All manifests have proper namespace declarations
- [x] Secrets use base64 encoding with warning comment
- [x] Resources have requests/limits as per architecture docs
- [x] Security contexts properly configured per component
- [ ] **Minor**: Add `##` help comments to Makefile k8s-* targets for discoverability
- [ ] **Minor**: Consider aligning ConfigMap names with filenames (`api-config` → `api-configmap`)
- [ ] **Future**: Replace hardcoded secrets with external secret management (Phase 2)

### Security Review

| Aspect | Status | Notes |
|--------|--------|-------|
| runAsNonRoot | ✅ | API (UID 1000), Frontend (UID 1001), Postgres (default) |
| readOnlyRootFilesystem | ✅ | Enabled for API container |
| allowPrivilegeEscalation | ✅ | Disabled for API |
| Secrets handling | ⚠️ | Base64 in Git - acceptable for Phase 1 dev, documented |
| Network exposure | ✅ | ClusterIP only, no external access |

### Performance Considerations

| Component | Requests | Limits | Assessment |
|-----------|----------|--------|------------|
| API | 128Mi/100m | 256Mi/500m | ✅ Appropriate for dev |
| Frontend | 64Mi/50m | 128Mi/200m | ✅ Appropriate for dev |
| PostgreSQL | 128Mi/100m | 256Mi/500m | ✅ Appropriate for dev |

### Test Architecture Assessment

- **Test Type**: E2E smoke tests via kubectl + curl
- **Coverage**: Full stack deployment, health checks, service connectivity
- **Test Data**: Database initialized via init-sql ConfigMap
- **Reliability**: Tests verified by Dev Agent during implementation
...
**Gate: PASS** → `docs/qa/gates/1.8-kubernetes-manifests.yml`

### Recommended Status

✅ **Ready for Done** - All acceptance criteria met, implementation is clean, and tests have passed. Minor improvements are optional and non-blocking.
